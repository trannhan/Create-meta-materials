...
class ScatteringIE_Collocation
{
    ...
    private:
        ...
        Complex        IntGCoef;  //Coefficient of solving integral of Green function
        Real           R2;        //Square radius of the circle inscribed in a sub cube
    ...
    public:
        ...
        virtual ~ScatteringIE_Collocation()
        {
        ...
        {            
            ...
            h = (h1+I*h2);
            ha2k = PI4*h*Distribution;
            BoundaryImpedance = h/pow(ParticleRadius,Kappa);
            Real R = SmallCubeSize/2;
            R2 = pow(R,2);
            IntGCoef = (PI4/K)*(-R*cos(K*R)/K + sin(K*R)/(K*K));
        }
...
*/
	 ...
	 ///////////////////////////////////////////////////////////////////
        ...
        inline const Complex IntGreen(Long s, Long t)
        {
            // Integrate Green function in R^3            

            // Square distance from cube s to cube t in R^3             
            Real xs,ys,zs,xt,yt,zt;
            Cube2Position(s,xs,ys,zs);
            Cube2Position(t,xt,yt,zt);          
            Real st2 = (pow(xs-xt,2)+pow(ys-yt,2)+pow(zs-zt,2)); 
            Real st = sqrt(st2); 
            ...
            if(st2<=R2)
               ...
               IntG = IntGCoef*cexp(I*K*st)/st;
	     //cout<<"\nnorm(IntG["<<s<<","<<t<<"])="<<cnorm<Real, Complex>(IntG);
